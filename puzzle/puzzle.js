...
let ulPuzzle = document.querySelectorAll('li');;
...
const getEmptyCell = () => {
    ...
    console.log('empty index',state.emptyCellIndex);
...
const setDraggable = (items) => {
    ...
    if(state.dimension[row][col-1]) {
        left = state.dimension[row][col-1];
    } 
...
const fillGrid = (items, tiles) => {
    console.log('fillGrid',items);
    let shuffled = shuffle(tiles);
    // shuffle the tiles arraay until there is a combination that is solvable
